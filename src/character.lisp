  ...
  (flet ((fmt-attr-mod (modifyer)
	   (cond 
	     ...
	     ((< modifyer 0)
	      (format nil "(~a)" modifyer))
	     (t ""))))
    (format t "Name: ~a~%" (character-name character))
    (format t "Class: ~a~%" (character-class character))
    (format t "Level: ~a~%" (character-level character))
    (format t "HP: ~a~%" (total-hit-points character))
    
    (format t "Str: ~a ~a~%" (character-strength character)
	    (fmt-attr-mod (attribute-modifier character 'strength)))
    (format t "Int: ~a ~a~%" (character-intelligence character)
	    (fmt-attr-mod (attribute-modifier character 'intelligence)))
    (format t "Wis: ~a ~a~%" (character-wisdom character)
	    (fmt-attr-mod (attribute-modifier character 'wisdom)))
    (format t "Dex: ~a ~a~%" (character-dexterity character)
	    (fmt-attr-mod (attribute-modifier character 'dexterity)))
    (format t "Con: ~a ~a~%" (character-constitution character)
	    (fmt-attr-mod (attribute-modifier character 'constitution)))
    (format t "Cha: ~a ~a~%" (character-charisma character)
