...
| --- | --- | --- | --- |
| Player creation | `:player` source form, player save/load, inventory data model, adaptation player creation helper, and browser player-state serialization. | A player-facing character creation flow. | Generated dungeon entrance flow. |
...
| Inspect room | Paragraphs, entities, conditional choices, local/ref/global state, generated room rendering, resolved room result facts, claimable loot, generated exits, and combat options. | Rich generated room interactions beyond description, facts, loot, exits, and the minimal combat loop. | Generated dungeon entrance flow. |
| Encounters | Table result conventions can name encounters; CL can create persistent encounter state, render active choices, attack, damage, ration use, defeat, and flee in the narrative flow. | Richer enemy profiles, morale, initiative/turn policy, and encounter rewards. | Generated dungeon entrance flow. |
| Loot and supplies | Table result conventions, player inventory helpers, gold field, resolver support for gold/item/supply mutations, player-facing generated-room loot claims, and browser inventory mutation. | Richer item descriptions, treasure tags, and reward policies. | Generated dungeon entrance flow. |
...
| Recovery | Player HP/max HP, fatigue, conditions, Deprived predicate, recovery helper, ration procedure, and browser item-use choices. | Full rest procedures and broader condition rules. | Generated dungeon entrance flow. |
...
| Browser presentation | Browser runtime stays narrative-first: prose and choices are the primary display, while player and encounter state remain serialized for moments that need them. | Player-facing character creation and contextual character/status reveals. | Character creation flow. |
...
6. **Browser player and encounter state**
   The browser backend serializes player and encounter state for save/load,
   generated-room actions, loot claims, ration use, and combat choices. The UI
   remains narrative-first: character stats and inventory should appear during
   creation or when a room/action explicitly asks for them, not as a permanent
   side panel.
